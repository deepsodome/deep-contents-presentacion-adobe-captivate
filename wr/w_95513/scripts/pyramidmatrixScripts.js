		...
		for(i=0;i<cpWidget.length;i++){
			for(j=0;j<cpWidget[i].children.length;j++){
				if(cpWidget[i].children[j].children[0] != undefined){
					if(cpWidget[i].children[j].children[0].contentDocument.getElementById("pyramidmatrixwdgt") != null){
...
function resizeInteractionPresenter(thewidth,theheight) {
	...
	var pyramidTop = ((contentBgHeight/2)-(pyramidHolderH/2)-(headerActiveSize))+(100+(50*scaleH));
	var pyramidLeft = ((contentBgWidth/2)-(pyramidHolderW/2)-(175*scaleW));
	...
	if (generalStyles.headerActive == 2) {
		...
		$(".scroll-pane").css('left',(pyramidHolderW+(40+(35*scaleW))))
	}else{
		//Resize and display content
...
function resizeInteraction(thewidth,theheight) {
	...
	//Adjust top and left positions
	var pyramidTop = ((contentBgHeight/2)-(pyramidHolderH/2)-(headerActiveSize))+(80+(50*scaleH));
	var pyramidLeft = ((contentBgWidth/2)-(pyramidHolderW/2)-(145*scaleW));
	$("#pyramidHolder").css("margin-top",pyramidTop);
	$("#pyramidHolder").css("margin-left",pyramidLeft);
	...
	if(parseInt(thewidth)<parseInt(theheight)){
		...
		if (generalStyles.headerActive == 2) {
			...
			//Adjust top and left positions
			var pyramidTop = ((contentBgHeight/2)-(pyramidHolderH/2)-(70*scaleH));
			var pyramidLeft = ((contentBgWidth/2)-(pyramidHolderW/2)-(60*scaleW));
			var tempHeight = 100
			var tempWidth = 55
			if(thewidth<=360){
				$("#pyramidHolder").css("margin-top",pyramidTop+10);
				tempHeight = 55
				tempWidth = 70
			}else{
				$("#pyramidHolder").css("margin-top",pyramidTop);
			...
			//Adjust top and left positions
			var pyramidTop = ((contentBgHeight/2)-(pyramidHolderH/2)-(headerActiveSize*scaleH)+(17+(25*scaleH)));
			var pyramidLeft = ((contentBgWidth/2)-(pyramidHolderW/2)-(10*scaleW));
			var tempHeight = 100
			var tempWidth = 55
			if(thewidth<=360){
				$("#pyramidHolder").css("margin-top",(75*fscaleH));
				tempHeight = 50
				tempWidth =  70
			}else{
				$("#pyramidHolder").css("margin-top",pyramidTop);
			...
			//Resize and display content
			$(".scroll-pane").css('width',revealWidth-(tempWidth*scaleW));
			$(".scroll-pane").css('height',(revealHeight-pyramidHolderH)-(110*fscaleH));
			$(".scroll-pane").css('left',(12+(25*scaleW)));
